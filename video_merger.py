...
def concatenate_sessions(target_directory, output_directory):
    ...
    # output_folder = os.path.join(output_directory, "concatenated_sessions")  # Removed this line
    ...
    for video_file in video_files:
        ...
        video_timestamp = datetime.datetime.strptime(
            f"{filename_parts[0]}_{filename_parts[1]}_{filename_parts[2]}", "%Y_%m%d_%H%M%S")

        print(f"Processing: {video_file}")  # Debug print
        ...
        if last_video_end is None:
            ...
            print(f"  - Starting a new session")  # Debug print
        else:
            time_difference = video_timestamp - last_video_end
            if time_difference.total_seconds() <= 60:
                current_session.append(video_file)
                last_video_end = video_timestamp + datetime.timedelta(seconds=video_duration)
                print(f"  - Adding to current session")  # Debug print
            else:
                if time_difference.total_seconds() > 300:  # 5 minutes
                    ...
                    session_file_path = os.path.join(output_directory,
                                                     session_file_name)  # Changed to output_directory
                    with open(session_file_path, "w") as session_file:
                        for file_to_concatenate in current_session:
                            session_file.write(file_to_concatenate + "\n")
                    print(
                        f"  - Ending session, created session file: {session_file_name}")  # Debug print
    ...
    if current_session:
        session_file_name = f"{current_session[0].split('_')[0]}_{current_session[0].split('_')[1]}_{current_session[0].split('_')[2]}.txt"
        session_file_path = os.path.join(output_directory, session_file_name)  # Changed to output_directory
        with open(session_file_path, "w") as session_file:
            for file_to_concatenate in current_session:
                session_file.write(file_to_concatenate + "\n")
        print(f"  - Ending session, created session file: {session_file_name}")  # Debug print
